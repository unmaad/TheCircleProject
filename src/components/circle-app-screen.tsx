...
export default function AppCircles({ user, onLogout }: { user: any; onLogout?: () => void }) {
  ...
  return (
      ...
      <header className="p-4 bg-brand text-white flex justify-between items-center">
        <div >
            <span className="text-amber-600">⭕circles</span><br/>
            <span className="text-sm text-gray-600 px-5"> Small groups, less noise. Built for clarity.</span>
              </div>
              <div>
                  <span className="text-amber-600 px-2">{user.avatar} {user.name}</span>
                  <button
            onClick={() => {
              // clear persisted user and ephemeral name then call optional handler
              try { localStorage.removeItem('circlesUser'); } catch (err) { console.warn('logout: failed to remove persisted user', err); };
              if (onLogout) onLogout();
            }}
            className="text-sm px-1 py-1 text-white rounded-md"
            title="Logout"
          > Logout
                  </button>
              </div>
      ...
      <main className="max-w-5xl px-5 mx-auto mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Left: circle list */}
        <aside className="col-span-1 bg-white p-4 rounded-lg shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-amber-600">{circles.length} Spring Loops</h2>
          </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <button
                      onClick={(e: React.MouseEvent) => { e.stopPropagation(); joinCircle(c.id); }}
                      className="text-xs px-2 py-1 rounded-md border"
                    >
                      Join
                    </button>
                    <button
                      onClick={(e: React.MouseEvent) => { e.stopPropagation(); navigator.clipboard?.writeText(generateInviteLink(c.id)); }}
          ...
          {!selected ? (
            ...
            <div>
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-amber-600">{selected.name}</h3>
              ...
              <div className="mt-4 flex gap-2">
                <input
                  value={composeText}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setComposeText(e.target.value)}
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-4 w-full max-w-md">
            <h4 className="font-semibold text-amber-600">Create a new Circle</h4>
            <label className="block text-sm mt-3 text-black">Name</label>
            <input value={newCircleName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCircleName(e.target.value)} className="w-full text-gray-700 border-gray-600 px-3 py-2 rounded-md" />
            <label className="block text-sm mt-3 text-black">Description (optional)</label>
            <input value={newCircleDesc} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCircleDesc(e.target.value)} className="w-full text-gray-700 border-gray-600 px-3 py-2 rounded-md" />
